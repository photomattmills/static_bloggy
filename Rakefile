task :new_post, [:post_name, :image_dir] do |t, args|
    ...
    "![Alt text](http://photomattmills.com/images/#{args.image_dir}/#{file})"
    images = files.map {|file| "![Alt text](http://photomattmills.com/images/#{args.image_dir}/#{file})"} #in an array of strings as img tags
  time = Time.now
